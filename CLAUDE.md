...
## Map info box parity (MapView, TripMapWindow, CandidateMapWindow)
The Map tab (`MapView`, embedded `UserControl`), the Trip Plan map (`TripMapWindow`, standalone `Window`), and the candidate-review map (`CandidateMapWindow`, standalone `Window`, US43) are three separate, independent map surfaces, but their airport-info-box interaction — clicking an airport, dragging a box, and how it behaves while panning/zooming — must behave identically. All three share `Helpers/DraggableInfoBox` for this (position tracking, drag handling, the leader line shown once a box is dragged from its default spot); a plain `Border` on the map's own `Canvas` overlay, not a WPF `Popup` (a `Popup` is a separate top-level window, which is what used to require manual workarounds for minimize/window-move/tab-switch — see `requirements.md` US42/BUG-12). `CandidateMapWindow` only has a primary box (no Ctrl+click/secondary) since a flat candidate list has no "pair" concept the way a `TripMapWindow` leg's two endpoints do — that's the one deliberate interaction difference.
- If you change one window's click/drag/zoom behavior for info boxes, mirror it in the others, or extend `DraggableInfoBox` itself so all three pick it up for free — don't let them drift apart.
