...
## Map info box parity (MapView and TripMapWindow)
The Map tab (`MapView`, embedded `UserControl`) and the Trip Plan map (`TripMapWindow`, standalone `Window`) are two separate, independent map surfaces, but their airport-info-box interaction — clicking an airport, Ctrl+click / leg-line-click for a second airport, dragging either box, and how both behave while panning/zooming — must behave identically. Both share `Helpers/DraggableInfoBox` for this (position tracking, drag handling, the leader line shown once a box is dragged from its default spot); a plain `Border` on the map's own `Canvas` overlay, not a WPF `Popup` (a `Popup` is a separate top-level window, which is what used to require manual workarounds for minimize/window-move/tab-switch — see `requirements.md` US42/BUG-12).
- If you change one window's click/drag/zoom behavior for info boxes, mirror it in the other, or extend `DraggableInfoBox` itself so both pick it up for free — don't let them drift apart.
