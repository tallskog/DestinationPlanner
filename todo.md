...
- [x] **Precipitation radar overlay** — 🌧 Precip toggle + refresh button, top-left of the map (not the filter sidebar). Fetches the current frame from RainViewer's free public API on toggle/refresh only — no background polling, matching the flight-planning use case rather than a live weather feed. Named "precipitation" rather than "rain" since the radar also covers snow/sleet/hail. See US38 in requirements.md.
- [x] **Wind barb overlay** — 🎏 Wind toggle + refresh + flight-level dropdown (Surface through FL390), top-left of the map. Fetches a 16×11 grid (176 points, kept under Open-Meteo's own ~450-500 point URL-length ceiling) from Open-Meteo's free global API in a single batched request, then bilinearly interpolates (via u/v vector components, so direction wraps at 360° correctly) up to a 44×28 visual grid (1,232 barbs) for a dense flow-field look. Re-samples automatically (debounced ~600ms) after panning/zooming while the overlay is on. See US39 in requirements.md.
- [x] **Fixed wind barbs / airport popups spilling into the sidebar** — `SelectionOverlay` now clips to the map area, and airport popups close instead of floating over the sidebar once their airport pans/zooms out of view. See BUG-08 in requirements.md.
