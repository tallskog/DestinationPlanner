...
- `TripMapWindow` supports the same airport-info interaction as the Map tab: clicking an airport marker opens a popup with ICAO, name, runway list, and METAR (fetched via `IMetarService`, mirroring `MapView`'s Primary popup). Clicking a leg's connecting line — not just its endpoint markers — opens **both** that leg's airports at once (Primary + Secondary popups side by side), since a line inherently represents a pair. Each visible route line is paired with a wider invisible hit-target line (`Stroke=Transparent`, thicker `StrokeThickness`) so the line remains a comfortably clickable target without changing its visual thickness
  ...
  - Clicking empty map area (no marker, no line nearby) closes any open popup — mirroring `MapView.OnMapInfo`'s null-feature branch — since a WPF `Popup` doesn't dismiss itself on an outside click the way a native context menu does
  - Dragging the `TripMapWindow` itself (not just panning the map) keeps any open popup anchored to its airport: `LocationChanged` nudges `HorizontalOffset` on both popups to force WPF to recompute their screen position, since an `AllowsTransparency` `Popup` doesn't reposition automatically when its host window moves. Same underlying WPF quirk and same fix as the Map tab's `MapView.OnMainWindowLocationChanged`/`NudgePopup` (US17.7/US17.8) — `TripMapWindow` needed its own copy since it's a separate top-level `Window`, not an embedded `UserControl` inside the main window
...
## US42: Draggable airport info boxes (Map tab)
**As a user**, I want to be able to drag an airport info box to a new spot on the map, so that when two selected airports are close together I can separate their overlapping boxes, or uncover the dashed distance line between them — without losing track of which airport a moved box belongs to.

**Background:** This required first replacing the underlying WPF `Popup` with a regular `Border` hosted on `SelectionOverlay` (the same `Canvas` already used for the aircraft marker, wind barbs, and the primary/secondary distance line). A `Popup` is a separate top-level window, which is exactly why it previously needed manual workarounds for window-minimize, window-move, and z-order (US17.7/US17.8, and BUG-08's fix) — a `Canvas`-hosted `Border` is part of the same visual tree as the rest of the map, so those all become automatic and the workaround code was deleted:
...
- `MapView.OnMainWindowLocationChanged`/`NudgePopup` (reposition-on-window-move) — removed; a normal element already tracks its own window's movement.
...
- The `_tabPrimaryWasOpen`/`_tabSecondaryWasOpen` explicit close/reopen dance around `OnLoaded`/`OnUnloaded` (needed because a `Popup`'s own window doesn't hide when its host tab is switched away) — removed; `OnUnloaded` itself is gone, and `OnLoaded` just recomputes position defensively for the case where the map area was resized while the tab was hidden.

**Acceptance criteria:**
- `PrimaryInfoBox`/`SecondaryInfoBox` (renamed from `PrimaryPopup`/`SecondaryPopup`) are `Border` elements inside `SelectionOverlay`, each explicitly `IsHitTestVisible="True"`, while `SelectionOverlay` itself carries **no** `IsHitTestVisible="False"` (see BUG-12 below — a WPF ancestor's `IsHitTestVisible="False"` cannot be overridden by a descendant's `True`, so every other, purely-decorative child on the Canvas — wind barbs, the aircraft marker, the leader lines, the primary/secondary distance line and its label — is instead marked `IsHitTestVisible="False"` individually, so only the two boxes intercept clicks and the map still pans/selects normally everywhere else)
- Dragging (`InfoBox_MouseLeftButtonDown`/`MouseMove`/`MouseLeftButtonUp`/`LostMouseCapture` in `MapView.xaml.cs`) uses standard `CaptureMouse`/`ReleaseMouseCapture`; the dragged position is clamped so at least one full corner of the box stays within `MapCtrl`'s bounds, so a box can never be dragged somewhere it can't be recovered from
- Each box's dragged position is stored as `InfoBoxState.ManualOffset` — a screen-space vector from the airport's marker to the box's top-left corner, recomputed live during drag from the airport's *current* screen position (not a snapshot), so a box dragged mid-pan still lands in the intended spot relative to the map, not the marker's pre-drag position
- Once `ManualOffset` is set, a thin line (`PrimaryLeaderLine`/`SecondaryLeaderLine`, colored to match that box's border) is drawn from the airport's marker to the box; the line is hidden again whenever `ManualOffset` is `null` (a freshly-opened box at its default position doesn't need one)
- Selecting a new airport into a slot (`OpenInfoBox`) resets that slot's `ManualOffset` to `null`, so the box reappears at the default anchor-relative position rather than carrying over wherever the previous airport's box had been dragged to
- `SetBoxPosition` still hides the box (and its leader line) once its airport's marker pans off the visible map area, same as before this change (BUG-08) — this is independent of whether the box itself has been dragged
- The primary/secondary distance line + nm label (added to `SelectionOverlay` dynamically, after the two static info boxes) renders on top of the info boxes rather than behind them — Canvas z-order follows `Children` collection order, and the line/label are appended later — preserved deliberately per user feedback while testing this feature

**Coverage:** UI-only (mouse-drag interaction and WPF layout/visual-tree behavior) — verified manually by launching the app, not by automated tests, per CLAUDE.md's Testing section. `TripMapWindow`'s own popups (a separate, near-duplicate implementation — see US41's note on `TripMapWindow`) were **not** touched by this change and still use the old `Popup`-based approach, including the same latent "won't reopen after panning back into view" bug independently fixed on the Map tab's version.
